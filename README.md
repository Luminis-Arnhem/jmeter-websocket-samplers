...
The payload (request data) can also be loaded from file, in which case it is not interpreted at all, but send as is. Hence, JMeter variables cannot be used (or at least: will not be resolved) and binary content should be stored in binary files; e.g. not encoded in hex.
